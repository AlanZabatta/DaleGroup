defmodule DaleApp.Products.LogroMensual do
  @moduledoc """
  Resultado congelado de un ciclo de 30 dias para un empleado: cuanto sumo
  de puntos de logro (posicion en su podio de categoria + posicion en el
  podio de asistencia, escala 10/8/6/4), y si quedo como ganador del ciclo.

  Se escribe una vez por ciclo cerrado y no se recalcula. Si hay empate en
  el puntaje mas alto, todos los empatados quedan con es_ganador: true. Si
  el dueno desempata a mano, esa fila pasa a ganador_elegido_manualmente:
  true y el resto de los empatados de ese ciclo se apagan.
  """
  use Ecto.Schema
  ...
  def changeset(logro, attrs) do
    ...
    |> unique_constraint([:brand_id, :ciclo_inicio, :user_id],
