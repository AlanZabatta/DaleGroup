  ...
  def redeem(conn, params) do
    ...
    case params do
      %{"code" => code} ->
        ...
        cond do
          is_nil(claim) ->
            render(conn, :result, message: "QR inválido", success: false)

          is_admin ->
            case Claims.redeem_claim_admin(claim) do
              {:ok, _} -> render(conn, :result, message: "¡Canjeado! +100 puntos al usuario", success: true)
              {:error, :already_redeemed} -> render(conn, :result, message: "Este cupón ya fue canjeado", success: false)
              {:error, :expired} -> render(conn, :result, message: "Este cupón venció (más de 12hs)", success: false)
            end

          is_nil(cajero_brand) ->
            render(conn, :result, message: "No tenés una marca asignada", success: false)

          true ->
            case Claims.redeem_claim(claim, cajero_brand.id) do
              {:ok, _} -> render(conn, :result, message: "¡Canjeado! +100 puntos al usuario", success: true)
              {:error, :wrong_brand} -> render(conn, :result, message: "Este QR es de otra marca", success: false)
              {:error, :already_redeemed} -> render(conn, :result, message: "Este cupón ya fue canjeado", success: false)
              {:error, :expired} -> render(conn, :result, message: "Este cupón venció (más de 12hs)", success: false)
            end
        end
